...
@ComponentAttribute.Annotation(
        Name = "Notre Dame Naive Bayes Printer",
        DefaultLabel = "ND NB Printer Component",
...
@IOSpecAttribute.Annotation.__Multiple({
	@IOSpecAttribute.Annotation(IOType = IOSpecType.__Enum.Input, Name = "nb_output_file", 
			DataType = String.class, Description = "String file name")
	
})

public class ND_NaiveBayesPrinter extends BaseComponent{
	
	public ND_NaiveBayesPrinter(ComponentLogger log) {
		super(log);
	}
	...
	public void Compute() throws ComponentException {
		...
		String txtFile = (String) super.get_Workspace().Load("nb_output_file");
		if(null != txtFile){
